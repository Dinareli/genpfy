...
const projetos = [
  {
    ...
    descricao: "Sistema completo de gestão de tarefas e projetos para equipes",
    recursos: ["Autenticação", "Dashboard", "Relatórios"],
    cor: "#3b82f6"
  },
  {
    id: 2,
    ...
    recursos: ["Pagamentos", "Carrinho", "Admin"],
    cor: "#10b981"
  },
  {
    id: 3,
    ...
    descricao: "Portal educacional com cursos e avaliações online",
    recursos: ["Vídeos", "Quizzes", "Certificados"],
    cor: "#f59e0b"
  },
  {
    id: 4,
    ...
    descricao: "Sistema de agendamento médico e prontuários",
    recursos: ["Agenda", "Prontuários", "Notificações"],
    cor: "#ef4444"
  }
]
...
const statusColors = {
  ...
  "Em Desenvolvimento": "bg-warning text-warning-foreground", 
...
export default function Projetos() {
  ...
  const projetosFiltrados = projetos.filter(projeto => {
    const matchBusca = projeto.nome.toLowerCase().includes(busca.toLowerCase()) ||
          ...
          <Card className="card-glass animate-fade-in" style={{ animationDelay: '0.2s' }}>
            <CardContent className="p-4">
              <div className="text-2xl font-bold text-warning">
                {projetos.filter(p => p.status === "Em Desenvolvimento").length}
              </div>
              <div className="text-sm text-muted-foreground">Em Desenvolvimento</div>
            </CardContent>
          </Card>
          
          <Card className="card-glass animate-fade-in" style={{ animationDelay: '0.3s' }}>
            <CardContent className="p-4">
              <div className="text-2xl font-bold text-muted-foreground">
                {projetos.filter(p => p.status === "Planejamento").length}
              </div>
              <div className="text-sm text-muted-foreground">Em Planejamento</div>
            </CardContent>
          </Card>
        </div>
            ...
            >
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                  ...
                  <Badge 
                    className={`text-xs ${statusColors[projeto.status as keyof typeof statusColors]}`}
                  >
                    {projeto.status}
                  </Badge>
                </div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center justify-between pt-2 border-t border-border/50">
                  <div className="flex items-center space-x-1 text-xs text-muted-foreground">
                    ...
                    <span>{new Date(projeto.dataCriacao).toLocaleDateString('pt-BR')}</span>
                  </div>
